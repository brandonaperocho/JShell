...
/**
 * LS Class --> Prints out the contents of the directory specified or the name
 * of the file specified, or the contents of the current working directory if no
...
public class LS implements Command {
  ...
  boolean recursive; // true if recursive option specified
  int subDirs = 0; //number of subdirectories in directory specified

  /**
   * Constructor for use when ls without any arguments (standard case) Will set
   * reference directory to current working directory Contents will later be set
   * to the sub files/folders of reference
   * 
  ...
  /**
   * Constructor for use when ls is called with a path. (non-standard) Will set
   * reference directory to either root or current directory, depending on if
   * path is absolute path or relative path
   * 
  ...
  /**
   * Will return string of each item in contents array on a new line. 
   * 
   */
  private String print() {
    ...
    if (standard) {
      if (contents.length == 0) {
        return "\n"; // empty line for empty directories
      }
      String result = "";
      for (int i = 0; i < contents.length; i++) {
        result = result.concat(contents[i] + "\n");
      }
      return result;
    } else { // Non-standard, LS called with path as argument
      String result = "";
      if (!error) {
        result = result.concat(fullPath + ":\n"); // dir/file name b4 contents
      }
      if (contents.length == 0) {
  ...
  /**
   * Will set the contents to be printed to what they should be Sub
   * files/folders for a directory, file contents for a file, error message if
   * necessary
   * 
  ...
  /**
   * Implements execute method for this command. Sets contents, and then return
   */
  @Override
