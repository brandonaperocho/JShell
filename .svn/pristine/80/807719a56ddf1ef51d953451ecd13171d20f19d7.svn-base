...
public class ManCMDTest {
  ...
  public void testCatManual() {
    ...
    String expected = "cat -- Display the contents of the files provided"
  ...
  // Test when command is /# mv ----------------------------------------------
  @Test
  public void testMvManual() {
    ManCMD temp = new ManCMD("mv");
    String expected =
        "mv -- Moves file/directory from OldPath to NewPath. It will be "
            + "removed from OldPath. An example is as follows. /# mv /1 /2";
    assertEquals(expected, temp.getMvManual());
  }

  // Test when command is /# cp ----------------------------------------------
  @Test
  public void testCpManual() {
    ManCMD temp = new ManCMD("cp");
    String expected =
        "cp -- Copies a file/directory from OldPath to NewPath. It will NOT be"
            + " removed from OldPath. An example is as follows. /# cp /1 /2";
    assertEquals(expected, temp.getCpManual());
  }

  // Test when command is /# get ---------------------------------------------
  @Test
  public void testGetManual() {
    ManCMD temp = new ManCMD("get");
    String expected =
        "get -- Gets a file from a URL and adds it to the current "
            + "directory. An example is as follows. /# get http://www.google."
            + "ca/results/a1.txt";
    assertEquals(expected, temp.getGetManual());
  }

  // Test when command is /# grep --------------------------------------------
  @Test
  public void testGrepManual() {
    ManCMD temp = new ManCMD("grep");
    String expected =
        "grep -- Returns lines in files that contain a certain regex. Can"
            + " be used recursively or just for one file. An example is as "
            + "follows. /# grep \"[0-9]+\" /1/2/file.txt";
    assertEquals(expected, temp.getGrepManual());
  }

  // Test when command is /# !number -----------------------------------------
  @Test
  public void testNumberManual() {
    ManCMD temp = new ManCMD("!number");
    String expected =
        "!number -- Recalls the (number)th command in history, and then"
            + "will execute it. An example is as follows. /# !5";
    assertEquals(expected, temp.getNumberManual());
  }
}
