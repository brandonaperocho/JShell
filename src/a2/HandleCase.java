...
public class HandleCase {
  ...
  public Directory getPreviousCommand() {
    if (listOfInput.length == 1 && listOfInput[0].charAt(0) == '!') {
      ...
      if (listOfInput.length == 3  && listOfInput[0].charAt(0) == '!') { 
        // Redirection
        if (listOfInput[1].equals(">") || listOfInput.equals(">>")) {
        return getPrevCommRedirHelper();
        }
      } else {
        System.out.println ("Invalid amaount of arguments, please try again!");
      }
    }
    return mainDir;
  }
  ...
  /**
   * Redirection helper for PreviousCommand
   * @return mainDir - Main directory from JShell
   */
  private Directory getPrevCommRedirHelper () {
    listOfInput = Arrays.copyOfRange(listOfInput, 0, 1);
    DirectoryCommand temp = new PreviousCommand(listOfInput, mainDir,
        rootDir, historyListHC, pushDPopDStackHC);
    return temp.execute();
  }
